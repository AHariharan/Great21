...
public class GameDataAccessDAOImpl implements GameDataAccessDAO {
	...
	public void updateRummyStat(UpdatePlayerRummyStat request)
	{
		for(String nickname : request.getPlayerlist().keySet())
		{
			...
			transaction.setCash(stat.getCash());transaction.setCreatedDate(Calendar.getInstance().getTime());
			transaction.setGameInstanceId(request.getGameInstanceID());transaction.setUserid(userid);
			if(stat.getCash() > 0) transaction.setTransType("CREDIT");
			else transaction.setTransType("DEBIT");
